// auth.js
import { auth, signInWithEmailAndPassword, createUserWithEmailAndPassword, firebase } from './firebase.js';
...
export async function registerCharacterHandler() {
    const emailInput = document.getElementById('emailInput');
    const passwordInput = document.getElementById('passwordInput');
    const usernameInput = document.getElementById('usernameInput');
    const confirmPasswordInput = document.getElementById('confirmPasswordInput');
    const spawnLocationInput = document.getElementById('spawnLocationInput');
    const authErrorElement = document.getElementById('authError');

    if (authErrorElement) authErrorElement.textContent = ""; // Clear previous errors
    ...
    try {
        ...
        const spawnLocation = spawnLocationInput.value;

        if (!email || !password || !username || !confirmPassword) {
            if (authErrorElement) authErrorElement.textContent = "All fields except spawn location are required.";
            throw new Error("All fields except spawn location are required.");
        }

        if (password !== confirmPassword) {
            if (authErrorElement) authErrorElement.textContent = "Passwords do not match.";
            throw new Error("Passwords do not match.");
        }

        // createUserWithEmailAndPassword will set auth.currentUser internally
        ...
        if (user) {
            const characterData = {
                ...
                health: 100,
                maxHealth: 100,
                spawn: spawnLocation || "default" // Default spawn if not provided
            };

            // firebase.saveCharacter uses auth.currentUser internally
            await firebase.saveCharacter(characterData);
            
            return { user, characterData };
        } else {
            // This case should ideally not be reached if createUserWithEmailAndPassword throws an error
            // or resolves successfully (meaning user should be set).
            if (authErrorElement) authErrorElement.textContent = "Registration failed: User not created.";
            throw new Error("Registration failed: User not created.");
        }

    } catch (err) {
        console.error("Register Character Handler Error:", err);
        if (authErrorElement) {
            if (err.code) { // Firebase error codes
                 switch (err.code) {
                    case 'auth/email-already-in-use':
                        authErrorElement.textContent = "Registration failed: Email already in use.";
                        break;
                    case 'auth/weak-password':
                        authErrorElement.textContent = "Registration failed: Password is too weak (should be at least 6 characters).";
                        break;
                    case 'auth/invalid-email':
                        authErrorElement.textContent = "Registration failed: Invalid email address.";
                        break;
                    default:
                        authErrorElement.textContent = "Registration failed: " + err.message;
                }
            } else { // Non-Firebase errors (e.g., passwords don't match)
                authErrorElement.textContent = "Registration failed: " + err.message;
            }
        }
        throw err; // Re-throw for main.js to catch
    }
}
